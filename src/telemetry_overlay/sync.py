...
by more than a year), so the alignment is a user-supplied offset rather than something
derived from metadata.

``log_time = offset + video_time * scale``
...
a preset describes a *look* and is reused across flights, an offset belongs to one
video/log pair.
"""
...
class SyncModel:
    ...
    offset: float = 0.0
    scale: float = 1.0
    #: Free-form note, e.g. how the offset was found ("autosync", "manual anchor").
    note: str = ""
    source: Path | None = field(default=None, compare=False)

    def log_time(self, video_time: float) -> float:
        return self.offset + video_time * self.scale

    def video_time(self, log_time: float) -> float:
        return (log_time - self.offset) / self.scale
    ...
    def save(self, path: Path) -> Path:
        path = Path(path)
        payload = {"offset": self.offset, "scale": self.scale, "note": self.note}
