...
class FrameReader:
    ...
    def __init__(
        self, path: str | Path, info: VideoInfo | None = None, thread_type: str = "NONE"
    ) -> None:
        self.path = Path(path)
        self.info = info or probe_video(self.path)
        self._container = av.open(str(self.path))
        self._stream = self._container.streams.video[0]
        # Default "NONE", not "AUTO": FFmpeg's own multi-threaded decoder, combined
        # with the repeated container.seek() this class does for random access,
        # stalls for several *seconds* per seek when running on a background thread
        # alongside an active Qt event loop (reproduced consistently on Windows; a
        # purely sequential decode loop with AUTO threading, as export.py uses, is
        ...
        # Callers that only seek once and then read forward sequentially (autosync's
        # optical-flow pass) can pass "AUTO" instead to get multi-threaded decode.
        self._stream.thread_type = thread_type
        self._iterator = None
